...
structural_checks:
  - id: "SC-001"
    check: "squad.yaml exists (NOT squad.yaml)"
    action: |
      test -f squads/{name}/squad.yaml && ! test -f squads/{name}/squad.yaml
    on_fail: "ABORT - Must use squad.yaml, not squad.yaml"

  - id: "SC-002"
    check: "entry_agent field present"
    action: "grep -q 'entry_agent:' squads/{name}/squad.yaml"
    on_fail: "ABORT - Missing entry_agent in squad.yaml"

  - id: "SC-003"
    check: "tested field present"
    action: "grep -q 'tested:' squads/{name}/squad.yaml"
    on_fail: "ABORT - Missing tested field in squad.yaml"

  - id: "SC-004"
    check: "Entry agent has activation-instructions"
    action: |
      ENTRY=$(grep 'entry_agent:' squad.yaml | cut -d: -f2 | tr -d ' ')
...
1. Usou `squad.yaml` ao invés de `squad.yaml`
