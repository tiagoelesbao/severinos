...
| VETO-SQD-003 | Missing entry agent | squad.yaml has no `entry_agent` OR referenced agent file does not exist | HALT at PHASE 3. Cannot proceed without valid entry point. | Phase 3 |
| VETO-SQD-004 | Missing workspace integration | squad.yaml has no `workspace_integration.level` field | HALT at PHASE 3. Must declare level (none/read_only/read_write/workspace_first). | Phase 3 |
...
| V4 | squad.yaml invalid | tier_1_result.config_valid == false | All squads |
...
|---|----------|-------|-----------|
| 1 | Config | `squad.yaml` existe | `ls squads/{name}/squad.yaml` |
| 2 | Config | NAO e `squad.yaml` | Arquivo deve ser `squad.yaml` |
| 3 | Config | Campo `entry_agent` presente | `grep "entry_agent:" squad.yaml` |
...
| 9 | Template | squad.yaml baseado em template | Estrutura match com squad-tmpl.yaml |
...
[PHASE 1: STRUCTURE - TIER 1] (BLOCKING)
    -> T1-CFG: squad.yaml exists, valid, required fields
