...
- Read squad manifests yourself (`squad.yaml` / `squad.yaml`)
...
├── Read canonical squad metadata from squad.yaml (fallback: squad.yaml)
...
# Trigger: Changes to squads/*/squad.yaml or squads/*/squad.yaml
...
2. Reads each canonical `squad.yaml` (fallback: `squad.yaml`)
