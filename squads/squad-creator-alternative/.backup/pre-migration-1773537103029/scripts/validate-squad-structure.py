...
WORKSPACE_LEVELS = {"none", "read_only", "read_write", "workspace_first"}
...
def validate_structure(squad_path: Path) -> Dict[str, Any]:
    ...
    # T1-CFG-001: squad.yaml exists
    config_path = squad_path / "squad.yaml"
    if config_path.exists():
        result["checks"].append({"id": "T1-CFG-001", "status": "pass", "message": "squad.yaml exists"})

        # T1-CFG-002: Valid YAML
        config = read_yaml(config_path)
        if config:
            result["checks"].append({"id": "T1-CFG-002", "status": "pass", "message": "squad.yaml is valid YAML"})
            ...
            for field in required:
                ...
                if found:
                    ...
                    result["blocking_issues"].append(f"squad.yaml missing required field: {field}")
                    result["passed"] = False
            ...
            result["checks"].append({"id": "T1-CFG-002", "status": "fail", "message": "squad.yaml has invalid YAML"})
            result["blocking_issues"].append("squad.yaml has invalid YAML syntax")
            result["passed"] = False
    else:
        result["checks"].append({"id": "T1-CFG-001", "status": "fail", "message": "squad.yaml not found"})
        result["blocking_issues"].append("squad.yaml not found")
        result["passed"] = False
