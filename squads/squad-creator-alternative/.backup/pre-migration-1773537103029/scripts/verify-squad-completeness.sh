...
# SC-001: squad.yaml exists
if [ -f "$ROOT/squad.yaml" ]; then
  check_pass "squad.yaml exists"
else
  check_fail "squad.yaml does not exist"
fi

# SC-001b: NOT squad.yaml
if [ -f "$ROOT/squad.yaml" ]; then
  check_fail "squad.yaml exists (should be squad.yaml)"
else
...
if grep -q "entry_agent:" "$ROOT/squad.yaml" 2>/dev/null; then
  ENTRY_AGENT=$(grep "entry_agent:" "$ROOT/squad.yaml" | head -1 | sed 's/.*entry_agent:[[:space:]]*//' | tr -d '"' | tr -d "'")
  check_pass "entry_agent defined: $ENTRY_AGENT"
else
  check_fail "entry_agent not defined in squad.yaml"
  ENTRY_AGENT=""
fi
...
if grep -q "tested:" "$ROOT/squad.yaml" 2>/dev/null; then
  ...
  check_fail "tested field missing in squad.yaml"
fi

# SC-003b: version field present
if grep -q "version:" "$ROOT/squad.yaml" 2>/dev/null; then
  VERSION=$(grep "version:" "$ROOT/squad.yaml" | head -1 | sed 's/.*version:[[:space:]]*//' | tr -d '"' | tr -d "'")
  check_pass "version defined: $VERSION"
else
  check_fail "version not defined in squad.yaml"
fi
...
if grep -qi "operational\|utility\|simple\|tool" "$ROOT/squad.yaml" 2>/dev/null; then
  IS_OPERATIONAL=true
fi
