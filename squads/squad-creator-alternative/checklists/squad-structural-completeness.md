...
|-------|-----------|----------|
| `squad.yaml` existe | `ls squads/{name}/squad.yaml` | YES |
| NÃO é `squad.yaml` | Arquivo deve ser `squad.yaml` | YES |
| Campo `entry_agent` presente | `grep "entry_agent:" squad.yaml` | YES |
...
| Check | Validação | Blocking |
|-------|-----------|----------|
| squad.yaml baseado em template | Estrutura match com squad-tmpl.yaml | YES |
...
|-----------------|---------------------|---------------------|
| `squad.yaml` | `templates/squad-tmpl.yaml` | name, version, entry_agent, agents[] |
...
template_compliance_check:
  - id: "TC-001"
    check: "squad.yaml foi gerado a partir de squad-tmpl.yaml"
...
NÃO carrega squad-tmpl.yaml
NÃO carrega agent-tmpl.md
Resultado: estrutura fora do padrão AIOX

✅ CORRETO:
1. Carregar template: Read(templates/squad-tmpl.yaml)
2. Preencher campos do template
3. Escrever resultado: Write(squads/{name}/squad.yaml)
```
...
if [ ! -f "$ROOT/squad.yaml" ]; then
  echo "❌ FAIL: squad.yaml não existe"
  ERRORS=$((ERRORS+1))
fi

if [ -f "$ROOT/squad.yaml" ]; then
  echo "❌ FAIL: squad.yaml existe (deveria ser squad.yaml)"
  ERRORS=$((ERRORS+1))
fi

if ! grep -q "entry_agent:" "$ROOT/squad.yaml" 2>/dev/null; then
  echo "❌ FAIL: entry_agent não definido em squad.yaml"
  ERRORS=$((ERRORS+1))
fi

if ! grep -q "tested:" "$ROOT/squad.yaml" 2>/dev/null; then
  echo "❌ FAIL: campo 'tested' não definido em squad.yaml"
  ERRORS=$((ERRORS+1))
fi
...
ENTRY=$(grep "entry_agent:" "$ROOT/squad.yaml" 2>/dev/null | cut -d: -f2 | tr -d ' ')
if [ -n "$ENTRY" ]; then
...
phase_0_structural:
  ...
  blocking: true
  checks:
    - "squad.yaml exists (NOT squad.yaml)"
...
rule: |
  ...
  1. Verificar squad.yaml (não squad.yaml)
