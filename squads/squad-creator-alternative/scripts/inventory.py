...
def read_config_yaml(squad_path: Path) -> Optional[Dict]:
    """Read and parse squad.yaml"""
    config_file = squad_path / "squad.yaml"
...
def scan_squad(squad_path: Path) -> SquadInventory:
    ...
    has_config = (squad_path / "squad.yaml").exists()
    has_readme = (squad_path / "README.md").exists()
    has_changelog = (squad_path / "CHANGELOG.md").exists()

    # Track issues
    if not has_config:
        issues.append({"type": "BLOCKING", "message": "Missing squad.yaml"})
    if not has_readme:
    ...
    if config_name and config_name != squad_path.name:
        issues.append({
            "type": "WARNING",
            "message": f"squad.yaml name '{config_name}' doesn't match folder '{squad_path.name}'"
        })
...
def format_output(inventory: SquadInventory, format: str) -> str:
    ...
    if format == "json":
        ...
        lines = [
            ...
            "",
            "Required Files:",
            f"  squad.yaml:  {'✓' if inventory.has_config_yaml else '✗ MISSING'}",
