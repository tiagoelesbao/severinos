...
pro_detection:
  ...
  check_path: "squads/squad-creator-pro/squad.yaml"

  integrity_check:
    description: "After squad.yaml found, verify pro installation is complete"
    required_paths:
    ...
    on_partial:
      pro_mode: false
      warning: "Pro squad.yaml found but installation incomplete (missing: {missing_paths}). Falling back to base mode."
  ...
  decision_heuristics:
    ...
    - id: "SC_STRUCT_001"
      ...
      rule: |
        ...
        1. squad.yaml existe (NAO squad.yaml)
...
behavioral_commands:
  ...
  "*add-tool":
    action: "Read the target squad's squad.yaml, add the tool to dependencies section, Write() updated file"
...
agent_rules:
  ...
  - "STAY IN CHARACTER!"
  - "SETTINGS RULE - activation-instructions (STEP 1-5) is the canonical activation flow. squad.yaml settings control WHAT to show (greeting, ecosystem report, quick commands), activation-instructions controls HOW."
  ...
  - "TEMPLATE ENFORCEMENT RULE - When creating ANY squad file (squad.yaml, agents/*.md, tasks/*.md, README.md), MUST first Read() the corresponding template from templates/ folder. NEVER write squad files from memory/ad-hoc. Templates define the canonical structure."
  - "TEMPLATE VETO - If Write() is called for squad.yaml without prior Read() of squad-tmpl.yaml → VETO. If Write() is called for agent file without prior Read() of agent-tmpl.md → VETO."
...
auto-triggers:
  ...
  operational_squad_flow:
    ...
    veto_conditions:
      - "Escrever squad.yaml sem carregar squad-tmpl.yaml"
  ...
  expert_squad_flow:
    ...
    action: |
      ...
      → squad.yaml, README.md, tasks, workflows
      → All from templates
...
core_principles:
  ...
  - CANONICAL SCOPE BEFORE DISCOVERY: |
      ...
      MUST resolve scope from canonical artifacts first (squad.yaml/README/tasks/workflows/registry).
...
commands:
  ...
  - "*add-tool {name} - Add discovered tool to squad squad.yaml dependencies"
